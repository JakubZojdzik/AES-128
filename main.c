...
byte *expandKey(byte *key)
{
    ...
    for (int i = 1; i < 11; i++)
    {
        ...
        res[i * 16] ^= res[(i - 1) * 16];
        res[i * 16 + 1] ^= res[(i - 1) * 16 + 1];
        res[i * 16 + 2] ^= res[(i - 1) * 16 + 2];
        res[i * 16 + 3] ^= res[(i - 1) * 16 + 3];
        ...
        for (int j = 1; j <= 3; j++)
        {
            ...
            res[i * 16 + j * 4 + 2] = res[i * 16 + (j - 1) * 4 + 2] ^ res[(i - 1) * 16 + j * 4 + 2];
            res[i * 16 + j * 4 + 3] = res[i * 16 + (j - 1) * 4 + 3] ^ res[(i - 1) * 16 + j * 4 + 3];
        }
    }
    return res;
}
...
byte *subBytesInv(byte state[16])
{
    byte *res = malloc(16);
    for (int i = 0; i < 16; i++)
    {
        res[i] = sub_box_inv[state[i]];
    }
    return res;
}
...
byte *shiftRows(byte state[16])
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        res[i + 8] = state[i + 4 * ((2 + i) % 4)];
        res[i + 12] = state[i + 4 * ((3 + i) % 4)];
    }
    return res;
}

byte *shiftRowsInv(byte state[16])
{
    byte *res = malloc(16);
    for (int i = 0; i < 4; i++)
    {
        res[i] = state[i + 4 * ((4 - i) % 4)];
        res[i + 4] = state[i + 4 * ((5 - i) % 4)];
        res[i + 8] = state[i + 4 * ((6 - i) % 4)];
        res[i + 12] = state[i + 4 * ((7 - i) % 4)];
    }
    return res;
}
...
byte *mixColumns(byte state[16])
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        byte c = state[i * 4 + 2];
        byte d = state[i * 4 + 3];
        res[i * 4] = mul2_box[a] ^ mul3_box[b] ^ c ^ d;
        res[i * 4 + 1] = a ^ mul2_box[b] ^ mul3_box[c] ^ d;
        res[i * 4 + 2] = a ^ b ^ mul2_box[c] ^ mul3_box[d];
        res[i * 4 + 3] = mul3_box[a] ^ b ^ c ^ mul2_box[d];
    }
    return res;
}
